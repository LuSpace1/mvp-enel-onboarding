      ...
      >
        ...
        >
          ...
          <motion.div variants={cardHija} className="z-10">
            <div
              className="group border-enel-blue relative w-80 rounded-[2rem] border-2 bg-white p-6 text-center shadow-xl"
          ...
          <motion.div
            variants={cardHija}
            className="relative z-10 my-3 flex w-full max-w-4xl flex-col items-center px-4"
          >
            <div className="flex max-w-full flex-wrap items-center justify-center gap-1.5 rounded-full border border-neutral-200/80 bg-white/85 p-1.5 shadow-xs backdrop-blur-md">
              <span className="text-enel-blue hidden border-r border-neutral-200 px-3 text-[9px] leading-none font-extrabold tracking-wider uppercase sm:inline-block">
                Áreas Staff
              </span>
              {areasStaff.map((area) => (
                <button
                  ...
                  className="group/staff hover:border-enel-blue/50 flex cursor-pointer items-center justify-center rounded-full border border-neutral-200/60 bg-white px-4 py-1.5 text-center whitespace-nowrap transition-all duration-300 hover:shadow-xs active:scale-95"
                >
                  <span className="text-enel-navy group-hover/staff:text-enel-blue text-[11px] leading-none font-bold tracking-wide transition-colors duration-300">
                    {area.nombre}
                  </span>
                </button>
              ))}
            ...
            {subgerencias.map((sub, idx) => (
              ...
              >
                ...
                <div
                  ...
                  className={`hover:border-enel-blue relative flex w-full cursor-pointer flex-col items-center rounded-[1.5rem] border-2 bg-white p-5 text-center shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${
                    nodoAbierto === sub.id
                  ...
                  <div className="bg-white p-5 text-left">
                    {videoDeSeccion(sub.videoSection) && (
                      <button
                        ...
                        className="group/btn bg-enel-blue hover:bg-enel-blue-dark mb-4 flex w-full transform cursor-pointer items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-[11px] font-extrabold text-white shadow-sm transition-all duration-300 hover:shadow-md active:scale-[0.98]"
                      >
                        <Play
                          size={14}
                          weight="fill"
        ...
        {videoActivo && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-3xl overflow-hidden rounded-[2rem] border border-neutral-800 bg-neutral-950 p-3 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Botón Cerrar */}
              <button
        ...
        {staffActivo && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95, y: 20 }}
              transition={{ type: 'spring', duration: 0.4, bounce: 0.15 }}
              className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border border-neutral-200 bg-white p-6 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Botón Cerrar */}
              <button
              ...
              <div className="mt-2 text-left">
                <span className="bg-enel-blue/10 text-enel-blue rounded-full px-3.5 py-1 text-[10px] font-extrabold tracking-wider uppercase">
                  Soporte Transversal Staff
                </span>
                <h3 className="text-enel-navy mt-3 mb-4 text-2xl font-bold">
                  {staffActivo.nombre}
                </h3>
                <div className="bg-enel-mist rounded-2xl border border-neutral-100 p-5">
